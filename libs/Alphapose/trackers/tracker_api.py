...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, temp_feat, pose,crop_box,ps,buffer_size=30):
...
class Tracker(object):
    def __init__(self, opt, device, gpus=[0]):
        '''
        ...
        self.model = nn.DataParallel(m,device_ids=gpus).to(device).eval()
    ...
    def update(self,inps=None,bboxs=None,pose=None,cropped_boxes=None,pscores=None,_debug = False):
        #bboxs:[x1,y1.x2,y2]
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        ...
        if len(bboxs)>0:
            detections = [STrack(STrack.tlbr_to_tlwh(tlbrs[:]), 0.9, f,p,c,ps,30) for

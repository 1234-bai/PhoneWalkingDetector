...
from libs.yolov5.yolov5DetectorApi import TargetsDetector, TargetsAnnotator, select_device, save_one_box
...
dataset = test.loadData(source='stgcnTrainData/Mscoco/PlayWithOneHand')
...
for path, _, im0s, vid_cap, s in dataset:
    ...
    if(len(peopleXyxyBoxes) > 0):
        ...
        for i,pose in enumerate(poses): #   对于每个有姿态的人像人像
            ...
            _, phoneBoxes, _ = phoneTest.detectorSingleImg(crop, classes=[0], conf_thres=0.4)
